...
| Core value (owner, 2026-09-12) | Ближе к «без акустики покупаю красивую плёнку, а шум сверху останется», но шире: **в моей/клиентской комнате** понять, что комфорт сейчас понижен; MultiFrame может поднять класс на ступень+; проверить ориентиры «в моей ситуации»; **почувствовать** насколько тише; понять **за что ~500 тыс.** и что даёт MultiFrame **помимо** звукоизоляции (бескаркас / сразу красивый+тихий потолок); менеджеру — **быстрый аргумент продажи**. |
| Audiences (revised 2026-09-13) | **Не сводить к двум ролям.** Разнообразие входов и ролей зафиксировано ниже (user models). Все получают **одну полную картину**: исходное состояние потолка + эффект MultiFrame (звук) + прочие преимущества. Узкий переключатель «заказчик / шоурум» — **снят** как недостаточный. |
| User models (owner 2026-09-13) | **Клиент (B2C):** таргет «красивый потолок → сделай и тихим»; поиск «звукоизоляция потолка»; переход с сайта MultiFrame «проверь свой потолок»; переход с сайта партнёра по потолкам «нужна ли ЗИ под натяжной». **Профи (B2B/помощь клиенту):** менеджер в офисе продаж (аргумент доп.продажи MultiFrame); мастер потолков на объекте; замерщик (доп.продажа); дизайнер (заложить ЗИ в проект и аргументировать заказчику); и аналогичные. |
| Segmentation (draft for confirm) | **Не длинный список ролей.** 1–2 наводящих вопроса (контекст «для кого / зачем»), плюс опционально вход по UTM/рефереру для хука. Ответ меняет **формулировки/CTA**, не обрезает данные. Черновик вариантов — в PRODUCT_SPEC; ждём ОК владельца. |
| What is the first user journey that must work end to end? | Контекст (наводящий вопрос) → comfort framing → комната → сценарии шума → текущее состояние → до/после → аудио-контраст → профиль + CTA. На результате — **полная аргументация для всех**: исходное состояние + эффект MultiFrame + прочие преимущества + классы + цифры + графики + объяснения (см. PRODUCT_SPEC). |
...
| MultiFrame non-acoustic benefits (from stp-multiframe.ru / stp.store — for result pack) | Бескаркасный модульный монтаж; малая потеря высоты; быстрый монтаж (1 мастер, панель <1 мин — claim сайта); совместимость с натяжным/подвесным; шип-паз + подрезка; можно вести коммуникации; доп. теплоизоляция; сертификация/безопасность материалов. В UI — как понятные плюсы «помимо дБ», не как техпаспорт. |
