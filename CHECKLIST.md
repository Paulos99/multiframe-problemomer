...
| Hybrid class stuck fallback (2026-09-15) | Если полный класс СП не растёт (удар > В): **не** вести Итог строкой `Дискомфорт → Дискомфорт`. Показать каналы воздух/удар и честно: потолок не закрывает Lnw ≤ 60. Locked L1 + class-shift — только когда гибрид реально поднимается. |
...
| Result features copy (2026-09-13; rewrite 2026-09-17) | Без пункта Flat/Wave. Блок `Уникальность системы MultiFrame` — **таблица контраста**, не эссе: обычный натяжной vs MultiFrame (шум / монтаж / состав). |
...
| Processing ceremony (2026-09-17) | После последнего вопроса Room — fullscreen зелёный loading (~7 с, `Пропустить`): прогресс-бар + сменяющиеся статусы по центру. Без карточек/эквалайзера/классов. Назад с Итога не проигрывает. |
...
| Feature copy style (confirmed 2026-09-13; UI 2026-09-17) | Уникальность на результате — **короткий контраст**, не тезисы-абзацы. Title `Уникальность системы MultiFrame`. Три строки: шум, монтаж, состав. Не слоганы и не техдамп. Flat/Wave **не** показываем. |
