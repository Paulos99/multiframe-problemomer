...
| Room field order (confirmed 2026-09-13) | тип → площадь → перекрытие → пол сверху → тип дома → стадия → планируемый потолок → шумные соседи. |
| Room required fields (confirmed 2026-09-13) | Обязательны только **тип комнаты** и **площадь**. Перекрытие, пол сверху, тип дома, стадия, потолок, шумные соседи — можно `Не знаю`, «Далее» доступно. |
...
| Room = primary diagnostic (2026-09-13) | Обязательно: тип, площадь, перекрытие. Добавить: пол сверху, тип дома, стадия, планируемый потолок, шумные соседи (`Не знаю` · `Обычно тихо` · `Соседи сверху шумные`). Год постройки — **не в MVP**. |
