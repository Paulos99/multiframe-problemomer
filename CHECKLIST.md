...
| Room = primary diagnostic (2026-09-13) | Обязательно: тип, площадь, перекрытие. **Добавить:** пол сверху, тип дома, стадия объекта, планируемый потолок, **шумные соседи сверху** (не квиз «мешает ли»). «Кто сверху» — нет. Год/период постройки — **опциональный кандидат** (прокси эпохи + контекст норм; слабее плиты/пола). |
