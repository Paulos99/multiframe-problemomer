...
| What is the first user journey that must work end to end? | Роль → comfort framing → комната → сценарии шума → текущее состояние → до/после → аудио-контраст → профиль + CTA в калькулятор MultiFRAME / демо-заявка. Ощущение и класс комфорта — первично; цифры/графики — открытый вопрос воркшопа (см. PRODUCT_SPEC). Аудио продаёт разницу (демо-stubs с меткой «демо»). |
