...
| Result declutter (confirmed 2026-09-14) | Итог перегружен — **упростить UI**, суть оставить. Убрать полный SimCompare/графики/дубли CTA payload; цифры — **одна строка-ориентир**. Преимущества — короткий список тезис+фраза, без длинных карточек. |
| Audio on Result (confirmed 2026-09-14) | На Итоге **ровно 3 кнопки** — по одной на вид шума (воздух / удар / смешанный). Exemplars: `Лай собаки` · `Детский бег` · `Стиральная машина`. Одна кнопка играет До→После подряд. Полный список примеров из workshop — справочный, не UI. |
