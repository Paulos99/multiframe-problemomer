...
| Audiences (confirmed) | **Оба равны:** домовладелец/заказчик **и** менеджер шоурума. Один поток ответов; на старте — **переключатель роли** (два коротких хука). |
| What is the first user journey that must work end to end? | Роль → comfort framing → комната → сценарии шума → текущее состояние → до/после → аудио-контраст → профиль + CTA в калькулятор MultiFRAME / демо-заявка. На результате — **полная аргументация**: персонально + классы + цифры + графики + объяснения (см. PRODUCT_SPEC). Аудио продаёт разницу (демо-stubs с меткой «демо»). |
